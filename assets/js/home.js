...
(function () {
    ...
    function initPromoAccordion() {
        ...
        strip.addEventListener('click', function (e) {
            var panel = e.target.closest('.aj-promo-acc__panel');
            if (!panel || !strip.contains(panel)) return;
            var idx = parseInt(panel.getAttribute('data-index') || '0', 10);
            if (isNaN(idx)) return;

            var surf = e.target.closest('.aj-promo-acc__surface');
            if (surf && surf.classList.contains('aj-promo-acc__surface--link') && panel.classList.contains('is-active')) {
                return;
            }
            if (surf && surf.classList.contains('aj-promo-acc__surface--link') && !panel.classList.contains('is-active')) {
                e.preventDefault();
        ...
        function renderPreviews() {
            ...
            for (var i = 0; i < slides.length; i += 1) {
                ...
                html += '<button type="button" class="aj-promo-split__preview"' + style
                    ...
                    + (pImage !== ''
                        ? '<img src="' + esc(pImage) + '" alt="" loading="lazy" decoding="async" width="128" height="420">'
